...
class SimpleModel:
    ...
    def render_game(self, game: SnakeGame, frames_dir: str, generation: int):
        """
        Renders the game state and saves the frame as an image.

        :param game: The SnakeGame instance
        :param frames_dir: Directory to save the frames
        :param generation: The current generation number (for naming the frames)
        """
        ...
        while total_steps < max_steps:
            ...
            # Capture the game frame after each step
            self.save_frame(game, frames_dir, generation, total_steps)

    def save_frame(self, game: SnakeGame, frames_dir: str, generation: int, step: int):
        """
        Saves the current game frame as an image.
        """
        plt.figure(figsize=(10, 10))
        game.render()
        plt.title(f"Generation: {generation}, Step: {step}")
        frame_filename = frames_dir / f"gen_{generation}_step_{step}.png"
